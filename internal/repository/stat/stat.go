...
func (r *statRepository) GetReviewStats(ctx context.Context) (*model.ReviewStats, error) {
	...
	userStatsQuery := `
		...
		LIMIT 20
	`
	rows, err := r.db.Query(ctx, userStatsQuery)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	prStatsQuery := `
		...
		LIMIT 20
	`
	rows, err = r.db.Query(ctx, prStatsQuery)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
